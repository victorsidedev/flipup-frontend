...
test('API bases support the development proxy and environment-specific backend prefixes', () => {
    ...
    assert.equal(resolveApiBaseUrl(' https://backend.example.com/api/ '), 'https://backend.example.com/api');
...
test('invalid API configuration fails before requests can target the wrong endpoint', () => {
    for (const value of ['', ' ', '/', 'backend.example.com', '//backend.example.com', 'ftp://backend.example.com',
        'https://user:password@backend.example.com', 'https://backend.example.com?key=value', '/api#fragment', '/api?key=value']) {
