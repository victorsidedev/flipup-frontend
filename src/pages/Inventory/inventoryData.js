...
export function filterPurchaseItems(purchase, items, query, status) {
    ...
    return describeItems(items).filter(item => {
        ...
        const searchableText = [item.name, item.id, purchase.id, purchase.source,
            ...item.ancestors.map(parent => parent.name)].join(' ').toLowerCase();
...
export function applyPurchase(purchases, updatedPurchase) {
    ...
    return exists ? purchases.map(group => group.purchase.id === updatedPurchase.purchase.id ? updatedPurchase : group)
