...
test('resolves an unordered multi-level family without dropping standalone items', () => {
  ...
  assert.deepEqual(
    items[0].ancestors.map((item) => item.name),
    ['PC', 'GPU'],
  );
...
test('filters sold items using the backend status', () => {
  const purchase = { id: 1, source: 'Shop' };
  const items = [
    { id: 1, name: 'PC', parentId: null, sale: null, status: 'available' },
    { id: 2, name: 'GPU', parentId: 1, sale: { items: [{ itemId: 2 }], price: 0 }, status: 'sold' },
    {
      id: 3,
      name: 'Cooler',
      parentId: 2,
      sale: { items: [{ itemId: 2 }, { itemId: 3 }], price: 0 },
      status: 'sold',
    },
  ];
  assert.deepEqual(
    filterPurchaseItems(purchase, items, 'PC', 'sold').map((item) => item.id),
    [2, 3],
  );
  assert.deepEqual(
    filterPurchaseItems(purchase, items, '', 'available').map((item) => item.id),
    [1],
  );
});

test('posts a multi-item sale with expenses and surfaces backend conflict messages', async (context) => {
  const sale = {
    items: [
      { itemId: 5, allocatedPrice: 60 },
      { itemId: 6, allocatedPrice: null },
    ],
    kind: 'sale',
    soldDate: '2025-12-31',
    price: 125.25,
    expenses: [
      { type: 'payment_fee', amount: 4.5, description: 'Stripe fee', itemId: null },
      { type: 'shipping_fee', amount: 8, description: null, itemId: 5 },
    ],
  };
  ...
  assert.deepEqual(await createSale(sale), updatedPurchase);
  mock.mock.mockImplementation(async () => ({
    ok: false,
    json: async () => ({ detail: 'This item is already sold' }),
  }));
  await assert.rejects(createSale(sale), /already sold/);
});
...
test('replaces a purchase with server-computed statuses without removing children', () => {
  ...
  const updatedPurchase = {
    purchase: { id: 1 },
    items: [
      { id: 1, status: 'sold', sale: { items: [{ itemId: 1 }, { itemId: 2 }], price: 100 } },
      { id: 2, status: 'sold', sale: { items: [{ itemId: 1 }, { itemId: 2 }], price: 100 } },
    ],
  };
...
test('sends the selected calendar sale date unchanged', async (context) => {
  ...
  context.mock.method(globalThis, 'fetch', async (_url, options) => {
    assert.equal(JSON.parse(options.body).soldDate, '2025-12-31');
    return { ok: true, json: async () => purchase };
  });
  assert.deepEqual(
    await createSale({
      items: [{ itemId: 5, allocatedPrice: null }],
      kind: 'sale',
      soldDate: '2025-12-31',
      price: 125.25,
      expenses: [],
