...
export default function usePurchaseEditor({ purchaseDetails, onSaved, onDeleted, onClose }) {
  ...
  const [baseline, setBaseline] = useState(() =>
    purchaseDetails
      ? JSON.stringify(purchasePayload(purchaseDetails.purchase, purchaseDetails.items))
      : '',
  );
  ...
  async function confirmDeletion() {
    ...
    try {
      if (confirmation.kind === 'purchase') {
        ...
        setItems((previous) =>
          previous
            .filter((item) => !removed.has(item.id) && (!isSavedItem(item) || byId.has(item.id)))
            .map((item) =>
              byId.has(item.id)
                ? { ...byId.get(item.id), name: item.name, price: item.price }
                : item,
            ),
        );
        setBaseline(JSON.stringify(purchasePayload(result.purchase, result.items)));
