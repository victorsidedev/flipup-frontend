...
export default function PurchasePopup({
  ...
  return (
    <>
      <Dialog
        open={open}
        onClose={editor.busy ? undefined : toggleOpen}
        fullWidth
        maxWidth="md"
        aria-labelledby="purchase-title"
        slotProps={{ transition: { onEntered: focusSelectedItem } }}
      >
        <DialogTitle id="purchase-title">
          {isEditing ? `Edit purchase #${editor.purchase.id}` : 'Add Purchase'}
        </DialogTitle>
          ...
          >
            ...
            >
              <PurchaseForm
                ...
                onUpdatePurchase={editor.updatePurchase}
                autoFocus={selectedItemId == null}
              />
              <Typography
                variant="body2"
              ...
              >
                {editor.dirty
                  ? 'Save your changes to enable recording sales.'
                  : 'Record sales or manage items below.'}
              </Typography>
              <Button
                onClick={() => editor.addItem(null)}
        ...
        <DialogActions sx={{ flexWrap: 'wrap', px: 3, py: 2, gap: 1 }}>
          {isEditing && (
            <Button
              color="error"
              onClick={editor.requestPurchaseDeletion}
              disabled={editor.busy}
              sx={{ mr: 'auto' }}
            >
              Delete purchase
      ...
      {editor.confirmation && (
        <DeleteConfirmation
          ...
          busy={editor.busy}
          onCancel={editor.cancelDeletion}
